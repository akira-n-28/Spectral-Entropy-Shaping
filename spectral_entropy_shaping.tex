...
\paragraph{Deeper application to transformers.} Our ViT results (Experiments~14--18) show SES works on transformer block outputs, attention sub-layer outputs, and attention weight matrices, and that these targets are complementary. Experiment~18 demonstrates that Triple SES ($+2.65$~pp accuracy) jointly regularizes representation and attention-mechanism diversity. However, deeper integration remains unexplored: applying SES to \emph{per-head} representations (rather than aggregated across heads) could provide finer-grained control over attention head specialization. An explicit inter-head decorrelation term---penalizing the off-diagonal entries of the head-level covariance matrix---could more directly enforce head diversity. The finding that optimal $\beta$ differs between CNNs ($0.7$) and ViTs ($0.5$) suggests that architecture-aware $\beta$ selection may be important. Scaling to larger ViTs and language models (BERT, GPT) on standard NLP benchmarks would further establish the scope of SES for transformer architectures.
