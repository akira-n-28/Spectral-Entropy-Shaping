...
def get_device():
    if torch.cuda.is_available():
        ...
        print(f"[INFO] Memory: {torch.cuda.get_device_properties(0).total_mem / 1e9:.1f} GB")
...
def make_wide_resnet_28_10(num_classes=100):
    ...
    model = WideResNet(depth=depth, widen_factor=widen_factor, num_classes=num_classes)
    nparams = sum(p.numel() for p in model.parameters()) / 1e6
    print(f"  [WRN-{depth}-{widen_factor}] {nparams:.1f}M parameters")
    return model
...
def main():
    ...
    # ── Experiment 1: Periodic SES ───────────────────────
    periodic_results = experiment_periodic_ses(device)
    plot_periodic_ses(periodic_results, CFG.OUTPUT_DIR)
    ...
    for k, v in periodic_results.items():
        json_out["periodic_ses"][k] = {kk: vv for kk, vv in v.items() if kk != "history"}
    ...
    # ── Final summary ────────────────────────────────────
    print(f"\n{'=' * 70}")
    print(f"PHASE 3 COMPLETE — {elapsed/60:.1f} min ({elapsed/3600:.1f} h)")
    print(f"{'=' * 70}")

    print("\n┌─ PERIODIC SES ────────────────────────────────────────────────┐")
    baseline_time = periodic_results["k0"]["avg_epoch_time"]
    for k in sorted(periodic_results.keys(), key=lambda x: int(x[1:])):
        r = periodic_results[k]
        ov = (r["avg_epoch_time"] / baseline_time - 1) * 100
        print(f"│  {r['label']:<12} Acc: {r['best_acc']:.2f}%  Rob: {r['rob_mean']:.2f}%  "
              f"Time: {r['avg_epoch_time']:.1f}s (+{ov:.0f}%)        │")

    print("├─ WIDERESNET-28-10 ────────────────────────────────────────────┤")
